...
func (r *Raft) handleSnapshot(m pb.Message) {
	// Your Code Here (2C).
	var log = r.RaftLog // restore the RaftLog
	var meta = m.Snapshot.Metadata
	term, _ := r.RaftLog.Term(meta.Index)
	var message = pb.Message{MsgType: pb.MessageType_MsgAppendResponse, To: m.From, From: r.id, Term: r.Term}
